...
export default function HomeScreen() {
  ...
  const handleRegisterDoctor = async () => {
    ...
    try {
      ...
      Alert.alert('Success', 'Welcome to MedScript!');
    } catch (e) {
      console.error(e);
      Alert.alert('Error', 'Failed to register practitioner profile.');
    }
  };
    ...
    return (
      <SafeAreaView className="flex-1 bg-surface" edges={['top']}>
        <View className="px-6 py-4 flex-row items-center border-b border-outline_variant">
          ...
          <Text className="text-xl font-display font-extrabold text-primary ml-3">MedScript</Text>
        </View>
        <ScrollView contentContainerClassName="p-6 pb-24">
          <View className="mb-8">
            <Text className="text-primary font-bold tracking-wider text-xs uppercase mb-1">Welcome Setup</Text>
            <Text className="font-display text-4xl font-extrabold text-on_surface leading-tight mb-2">Practitioner Profile</Text>
          ...
          <View className="bg-surface_container_lowest p-6 rounded-xl border border-outline_variant space-y-6 shadow-sm">
            ...
            >
              <Text className="font-display font-bold text-white text-base">Complete Setup</Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
      </SafeAreaView>
    ...
    <SafeAreaView className="flex-1 bg-surface" edges={['top']}>
      <ScrollView contentContainerClassName="pb-24">
        ...
        <View className="px-6 py-4 flex-row items-center justify-between border-b border-outline_variant">
          ...
          <TouchableOpacity className="w-10 h-10 rounded-full bg-surface_container_high items-center justify-center">
